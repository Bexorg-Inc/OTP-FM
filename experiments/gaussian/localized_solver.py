"""
Per-sample localized OTP-FM solver (Sec. 4, Eqs. eq:otp_sample / eq:conditional_Xt).
...
This module is intentionally agnostic to the marginal family - it acts only on
sample-level coordinates. Trajectories satisfy the boundary conditions
X(0) = x_0 and X(1) = x_1 exactly by construction.
...
def _Phi(z):
    return 0.5 * (1.0 + math.erf(z / math.sqrt(2.0))) if np.isscalar(z) else 0.5 * (
        1.0 + np.erf(z / np.sqrt(2.0))
    )
...
def _phi(z):
    return (
        ...
        if np.isscalar(z)
        else np.exp(-0.5 * z * z) / np.sqrt(2 * np.pi)
    )
...
def double_integrated_lambda(t: ArrayLike, t_k: float, width: float, lambda_type: str) -> np.ndarray:
    """
    ...
    if lambda_type == "triangle":
        ...
        # Region 1: t <= a   -> Lambda = 0, I2 = 0 (already 0)
        ...
        #     I2(t) = I2(t_k) + integral_{t_k}^{t} Lambda(s) ds
        #     I2(t_k) = h / 6
        #     For 0 < u = t - t_k < h:
    ...
    if lambda_type == "box":
        ...
        m3 = t >= b
        if np.any(m3):
            # I2(b) = (2h)^2 / (4 h) = h, then linear in t past that.
            out[m3] = h + (t[m3] - b)
        return out
...
def localized_conditional_trajectories(
    ...
    lw_arr = np.broadcast_to(np.atleast_1d(np.asarray(lambda_width, dtype=float)), (K,)).astype(float)
